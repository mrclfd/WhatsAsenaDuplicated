...
Asena.addCommand({pattern: 'hidetag ?(.*)', fromMe: true, dontAddCommandList: true, desc: Lang.HIDETAG_DESC}, (async (message, match) => {
...
const rp = "*Melaporkan!*"
const ss = "Admin"
Asena.addCommand({pattern: 'tagadmin', fromMe: true, dontAddCommandList: true, desc: ss}, (async (message, match) => {
    let grup = await message.client.groupMetadata(message.jid);
    var jids = [];
    mesaj = '';
    grup['participants'].map(async (uye) => {
...
const rep = "*Harap balas Pesan Pengguna untuk Dilaporkan!*"
const rap = "=== ```Lapor``` ===\n\n"
Asena.addCommand({pattern: 'report ?(.*)', fromMe: true, dontAddCommandList: true, desc: tor}, (async (message, match) => {
    if (match[1] == '' && message.reply_message) {
        let grup = await message.client.groupMetadata(message.jid);
        var jids = [];
        mesaj = '';
