...
function secondsToHms(seconds) {
    ...
    var dDisplay = d > 0 ? d + (d == 1 ? " " + Lang.DAY + ", " : " " + Lang.DAY + ", ") : "";
...
Asena.addCommand({pattern: 'afk ?(.*)', fromMe: true, deleteCommand: true, desc: Lang.AFK_DESC}, (async (message, match) => {     
    if (!AFK.isAfk) {
...
Asena.addCommand({pattern: 'afk ?(.*)', fromMe: true, deleteCommand: true, dontAddCommandList: true, desc: Lang.AFK_DESC}, (async (message, match) => {     
    if (!AFK.isAfk) {
        AFK.lastseen = Math.round((new Date()).getTime() / 1000);
        if (match[1] !== '') { AFK.reason = match[1]; }
        AFK.isAfk = true;
